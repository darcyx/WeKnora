export default {
  platformApiKeys: {
    ...
    capabilityHints: {
      ...
      tokenQuotaRead: 'Просмотр квот и использования Token внешних пользователей указанного пространства.',
      tokenQuotaManage: 'Установка или сброс квот Token внешних пользователей указанного пространства.'
    },
    capabilities: {
      ...
      tokenQuotaRead: 'Просмотр квот Token внешних пользователей',
      tokenQuotaManage: 'Управление квотами Token внешних пользователей'
    }
  },
  tokenQuotaSettings: {
    title: 'Квоты Token внешних пользователей',
    description: 'Просматривайте использование и переопределяйте стандартные квоты Token внешнего пользователя в пространстве.',
    identityHint: 'Квота определяется сочетанием ID пространства и ID внешнего пользователя из заголовка API X-External-User-ID.',
    tenantId: 'ID пространства', tenantIdPlaceholder: 'Введите ID пространства', tenantIdRequired: 'Введите ID пространства', tenantIdInvalid: 'ID пространства должен быть числом',
    subjectId: 'ID внешнего пользователя', subjectIdPlaceholder: 'Введите значение X-External-User-ID', subjectIdRequired: 'Введите ID внешнего пользователя', query: 'Найти', listUsers: 'Список пользователей', usersTitle: 'Обнаруженные внешние пользователи', usersDescription: 'Показаны только пользователи с записью об использовании или переопределением квоты. Выберите пользователя для изменения квоты.', previousPage: 'Назад', nextPage: 'Далее', pageInfo: 'Страница {page}, пользователей: {total}',
    dailyUsage: 'Использовано за день', monthlyUsage: 'Использовано за месяц', reservedTokens: 'Зарезервировано', reservedHint: 'Выполняющиеся запросы временно занимают квоту',
    unlimited: 'Без лимита', limitValue: 'Лимит: {value}', overrideTitle: 'Переопределение квоты пользователя', overrideDescription: 'Пустой период наследует стандартную квоту платформы.',
    overridden: 'Переопределено', inheriting: 'Стандартное значение платформы', dailyLimit: 'Дневной лимит Token', monthlyLimit: 'Месячный лимит Token',
    inheritPlaceholder: 'Оставьте пустым для значения платформы', limitHint: 'Введите 0, чтобы снять лимит для этого периода.', reset: 'Восстановить по умолчанию',
    resetConfirm: 'Удалить переопределение квоты этого пользователя и восстановить значение платформы?', save: 'Сохранить квоту', noData: 'Записей об использовании квоты нет.',
    loadFailed: 'Не удалось загрузить квоту Token', overrideRequired: 'Укажите лимит хотя бы для одного периода', saveSuccess: 'Квота Token сохранена',
    saveFailed: 'Не удалось сохранить квоту Token', resetSuccess: 'Восстановлена стандартная квота платформы', resetFailed: 'Не удалось восстановить стандартную квоту платформы'
  },
  tenantInvitation: {
  ...
  system: {
    ...
    globalSettings: {
      ...
      keyDescriptions: {
        ...
        token_quota: {
          default_daily_limit: 'Стандартный дневной лимит Token для каждого внешнего пользователя в одном пространстве. 0 означает отсутствие лимита; для пользователя можно задать переопределение.',
          default_monthly_limit: 'Стандартный месячный лимит Token для каждого внешнего пользователя в одном пространстве. 0 означает отсутствие лимита; для пользователя можно задать переопределение.',
          max_completion_tokens: 'Стандартный верхний предел резерва для вывода, когда вызов не задаёт максимальное число Token. Он автоматически уменьшается до оставшейся квоты пользователя.'
        },
        asynq: {
      ...
      keyLabels: {
        ...
        token_quota: {
          default_daily_limit: 'Дневной лимит Token внешнего пользователя по умолчанию',
          default_monthly_limit: 'Месячный лимит Token внешнего пользователя по умолчанию',
          max_completion_tokens: 'Максимум выходных Token по умолчанию'
        },
        asynq: {
